...
class Renderer {
    constructor(scene, htmlCanvas) {
      ...
      this.depthTexture = ti.depthTexture(
    ...
    dirToUV = ti.func((dir) => {
      return [
        0.5 + (0.5 * Math.atan2(dir[2], dir[0])) / Math.PI,
        1.0 - Math.acos(dir[1]) / Math.PI,
      ];
    });
  
    tonemap = ti.func((color, exposure) => {
      let A = 2.51;
      let B = 0.03;
      let C = 2.43;
      let D = 0.59;
    ...
    ggxDistribution = ti.func((NdotH, alpha) => {
    ...
    async init() {
      ...
      if (this.scene.ibl) {
        this.iblLambertianFiltered = ti.texture(
          4,
          this.scene.ibl.texture.dimensions
        );
        this.iblGGXFiltered = ti.texture(
          4,
          this.scene.ibl.texture.dimensions.concat([16]),
          1,
          {
        ...
        await this.skyboxIBO.fromArray([
          0, 1, 2, 1, 3, 2, 4, 5, 6, 5, 7, 6, 0, 2, 4, 2, 6, 4, 1, 3, 5, 3, 7, 5,
          ...
          () => {
            ...
            let filterLambertian = (normal) => {
              ...
              for (let i of range(kSampleCount)) {
                ...
                let sampled = ti.textureSampleLod(
                  this.scene.ibl.texture,
                  halfDirCoords,
                  lod
                );
            ...
            let filterGGX = (normal, roughness) => {
              ...
              for (let i of range(kSampleCount)) {
                let xi = hammersley2d(i, kSampleCount);
                let importanceSample = getGGXImportanceSample(
                  normal,
                  roughness,
                  xi
                );
            ...
            for (let I of ti.ndrange(
              this.iblGGXFiltered.dimensions[0],
              this.iblGGXFiltered.dimensions[1]
            )) {
              let numLevels = this.iblGGXFiltered.dimensions[2];
              for (let level of range(numLevels)) {
                let roughness = level / (numLevels - 1);
                let uv =
                  I / (this.iblGGXFiltered.dimensions.slice(0, 2) - [1.0, 1.0]);
                let dir = this.uvToDir(uv);
            ...
            for (let I of ti.ndrange(
              this.LUT.dimensions[0],
              this.LUT.dimensions[1]
            )) {
      ...
      this.renderKernel = ti.classKernel(
        this,
        ...
        (camera) => {
          ...
          let evalSpecularBRDF = (
            ...
            let G2_Over_4_NdotL_NdotV =
              0.5 / lerp(2 * NdotL * NdotV, NdotL + NdotV, alpha);
          ...
          let evalBRDF = (material, normal, lightDir, viewDir, halfDir) => {
            ...
            let metallicBRDF = evalMetalBRDF(
              alpha,
            ...
            let dielectricBRDF = evalDielectricBRDF(
              alpha,
              material.baseColor.rgb,
              normal,
              lightDir,
            ...
            return (
              material.metallic * metallicBRDF +
              (1.0 - material.metallic) * dielectricBRDF
            );
          };
          ...
          let evalIBL = (material, normal, viewDir) => {
            ...
            if (ti.static(this.scene.ibl !== undefined)) {
              ...
              let diffuseLight = sRGBToLinear(
                this.tonemap(
                  ti.textureSample(this.iblLambertianFiltered, normalUV).rgb,
                  this.scene.ibl.exposure
                )
              );
              let diffuse = diffuseColor * diffuseLight;
  
              let specularColor =
                (1.0 - material.metallic) * dielectricF0 +
                material.metallic * material.baseColor.rgb;
              let reflection = normalized(
                2.0 * normal * dot(normal, viewDir) - viewDir
              );
              let reflectionUV = this.dirToUV(reflection);
              let specularLight = sRGBToLinear(
                this.tonemap(
                  ti.textureSample(
                    this.iblGGXFiltered,
              ...
              let scaleBias = ti.textureSample(this.LUT, [
                NdotV,
                material.roughness,
              ]).rg;
              let specular =
                specularLight * (specularColor * scaleBias[0] + scaleBias[1]);
          ...
          let getNormal = (normal, normalMap, texCoords, position) => {
            ...
            let temp =
              (uvDy.y * posDx - uvDx.y * posDy) /
              (uvDx.x * uvDy.y - uvDy.x * uvDx.y);
          ...
          for (let batchID of ti.static(
            ti.range(this.batchesDrawInfoBuffers.length)
          )) {
            let getMaterial = (texCoords, materialID) => {
              ...
              if (ti.static(this.batchInfos[batchID].materialIndex != -1)) {
                let materialRef =
                  this.scene.materials[this.batchInfos[batchID].materialIndex];
                if (ti.static(materialRef.baseColor.texture !== undefined)) {
                  let sampledBaseColor = ti.textureSample(
                    materialRef.baseColor.texture,
                    texCoords
                  );
                ...
                if (
                  ti.static(materialRef.metallicRoughness.texture !== undefined)
                ) {
                  let metallicRoughness = ti.textureSample(
                    materialRef.metallicRoughness.texture,
                    texCoords
                  );
                ...
                if (ti.static(materialRef.emissive.texture !== undefined)) {
                  let sampledEmissive = ti.textureSample(
                    materialRef.emissive.texture,
                    texCoords
                  ).rgb;
                ...
                if (ti.static(materialRef.normalMap.texture !== undefined)) {
                  let sampledNormal = ti.textureSample(
            ...
            for (let v of ti.inputVertices(
              ...
              this.batchesDrawInfoBuffers[batchID],
              this.batchesDrawInfoBuffers[batchID].dimensions[0]
            )) {
              let instanceIndex = ti.getInstanceIndex();
              let instanceInfo =
                this.batchesDrawInstanceInfoBuffers[batchID][instanceIndex];
              let nodeIndex = instanceInfo.nodeIndex;
              let materialIndex = instanceInfo.materialIndex;
              let modelMatrix =
                this.sceneData.nodesBuffer[nodeIndex].globalTransform.matrix;
  
              v.normal = ti
                .transpose(ti.inverse(modelMatrix.slice([0, 0], [3, 3])))
                .matmul(v.normal);
              v.position = modelMatrix.matmul(v.position.concat([1.0])).xyz;
              let pos = vp.matmul(v.position.concat([1.0]));
              ti.outputPosition(pos);
              let vertexOutput = ti.mergeStructs(v, {
          ...
          if (ti.static(this.scene.ibl !== undefined)) {
            ...
            for (let f of ti.inputFragments()) {
              ...
              color.rgb = linearTosRGB(
                this.tonemap(color.rgb, this.scene.ibl.exposure)
              );
              color[3] = 1.0;
    ...
    async computeDrawBatches() {
      ...
      for (let i = 0; i < this.scene.materials.length; ++i) {
        ...
        for (
          let nodeIndex = 0;
          nodeIndex < this.scene.nodes.length;
          ++nodeIndex
        ) {
        ...
        if (material.hasTexture()) {
          ...
          textureFreeBatchDrawInfo =
            textureFreeBatchDrawInfo.concat(thisMaterialDrawInfo);
          textureFreeBatchInstanceInfo = textureFreeBatchInstanceInfo.concat(
            thisMaterialInstanceInfo
          );
        }
      }
      if (
        textureFreeBatchDrawInfo.length > 0 &&
        textureFreeBatchInstanceInfo.length > 0
      ) {
      ...
      for (let drawInfos of this.batchesDrawInfos) {
        let buffer = ti.field(
          ti.engine.DrawInfo.getKernelType(),
          drawInfos.length
        );
      ...
      for (let drawInstanceInfos of this.batchesDrawInstanceInfos) {
        let buffer = ti.field(
          ti.engine.InstanceInfo.getKernelType(),
          drawInstanceInfos.length
        );
